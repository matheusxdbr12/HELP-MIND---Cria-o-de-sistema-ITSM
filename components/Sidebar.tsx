        ...
        <nav className="space-y-2">
          ...
          {hasPermission(AppPermission.VIEW_OWN_TICKETS) && (
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-lg flex items-center space-x-3 transition-colors ${activeView === 'my-tickets' || (activeView.startsWith('ticket-') && !hasPermission(AppPermission.VIEW_ALL_TICKETS) && !hasPermission(AppPermission.VIEW_DEPARTMENT_TICKETS)) ? 'bg-slate-800 text-indigo-400' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
              >
          ...
          {/* Agent / Admin Links */}
          {(hasPermission(AppPermission.MANAGE_SYSTEM) || hasPermission(AppPermission.MANAGE_USERS)) && (
             <button
          ...
          {(hasPermission(AppPermission.VIEW_ALL_TICKETS) || hasPermission(AppPermission.VIEW_DEPARTMENT_TICKETS)) && (
            <button
              onClick={() => setActiveView('dashboard')}
              className={`w-full text-left px-4 py-3 rounded-lg flex items-center space-x-3 transition-colors ${activeView === 'dashboard' || (activeView.startsWith('ticket-') && (hasPermission(AppPermission.VIEW_ALL_TICKETS) || hasPermission(AppPermission.VIEW_DEPARTMENT_TICKETS))) ? 'bg-slate-800 text-indigo-400' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
              <span>{t('ticketQueue')}</span>
            </button>
          )}
      ...
      <div className="p-4 bg-slate-950 flex flex-col space-y-4">
        ...
        <div className="flex bg-slate-800 rounded-lg p-1">
            <button 
                ...
                className={`flex-1 text-xs font-medium py-1.5 rounded-md transition-colors ${language === 'en' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
            >
                English
            </button>
            <button 
                onClick={() => setLanguage('pt')}
                className={`flex-1 text-xs font-medium py-1.5 rounded-md transition-colors ${language === 'pt' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
            >
                Português
            </button>
        </div>
